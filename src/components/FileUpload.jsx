...
export default function FileUpload({ title='Upload Identity Proof', type='id', onChange }){
  ...
  const handleFile = (e) => {
    ...
    setItems(next); onChange?.(next); inputRef.current.value='';
  };
    ...
    <div className="col-span-6">
      ...
      <div className="flex items-center gap-2 relative">
        <button type="button" className="btn">
          ...
          <input ref={inputRef} type="file" className="absolute inset-0 opacity-0 cursor-pointer" accept=".jpg,.jpeg,.pdf" onChange={handleFile} />
        </button>
      ...
      <div className="mt-3 space-y-2">
        {items.map(i=> (
          <div key={i.id} className="flex items-center gap-2 text-sm text-slate-700">
            ...
            <button type="button" className="ml-2 text-slate-500 hover:text-red-600" onClick={()=>remove(i.id)} title="remove">🗑</button>
          </div>
        ))}
      </div>
    </div>
